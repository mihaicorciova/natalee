...
class Client
{
    ...
    private static $path_prefix = '/api-3';
    ...
    private static function mapCollection($resource, $object)
    {
        ...
        self::$resource = class_exists($baseResource) ? $baseResource : 'Emag\\Api\\Resources\\' . $resource;
    ...
    private static function mapResource($resource, $object)
    {
        ...
        $baseResource = __NAMESPACE__ . '\\' . $resource;
        $class = class_exists($baseResource) ? $baseResource : 'Emag\\Api\\Resources\\' . $resource;
        return new $class($object);
    }
    ...
    {
        return self::createResource('/product_offer/save', $object);
    }
    ...
    {
        $filter = Filter::create($filter);
        return self::getCollection('/category/read' . $filter->toQuery(), 'Category');
    }
    public static function getCategoriesCount($filter = false)
    {
        $filter = Filter::create($filter);
        return self::getCount('/category/count' . $filter->toQuery());
    }
    public static function getCategory($id)
    {
        return self::getResource('/category/read' . $id, 'Category');
    }
    public static function createCategory($object)
    {
        return self::createResource('/category/save', $object);
    }
...
namespace Emag\Api\Resources;

use Emag\Api\Resource;
use Emag\Api\Client;
